...
const GleanCard = ({ context, actions }) => {
  ...
  const openGleanAgentIframe = async () => {
    ...
    try {
      ...
      // ⚠️ SECURITY NOTE: Hardcoded agent ID for testing - replace with env var for production
      const agentId = process.env.GLEAN_AGENT_ID || '5057a8a588c649d6b1231d648a9167c8';
  ...
  const openGleanAgentNewTab = () => {
    // ⚠️ SECURITY NOTE: Hardcoded agent ID for testing - replace with env var for production
    const agentId = process.env.GLEAN_AGENT_ID || '5057a8a588c649d6b1231d648a9167c8';
    const agentUrl = `https://app.glean.com/agents/${agentId}`;
